...
if ./rpgm $grid_length $grid_length $seed $simulation_time $speedStr $num_of_groups $num_of_nodes_per_group $speed_randomness $angle_randomness $param_str ; then
	...
	#if ns group/icdcs_random.tcl $grid_length $grid_length $seed $num_of_domains $num_of_groups_per_domain $num_of_gateways_per_group $num_of_nongateways_per_group $dynamic $percentage_gateway $interval_ebeacon $interval_ibeacon $simulation_time $param_str > $dump_file 2>/dev/null; then
