...
if {[llength $argv] != 1} {
    puts "Missing parameters... beacon"
    exit 1
}
...
set topo(ng)		 0; 	# max number of gateway nodes
set topo(dnn) 	 [expr $topo(nn) / $topo(nd)];

set topo(dynamic) 0;
set topo(percentage_gateway) 100;
set topo(eb)		 [lindex $argv 0];
set topo(ib)		 [lindex $argv 0];
...
set nf [open dump_files/t2.nam w]
$ns namtrace-all-wireless $nf $topo(grid_x) $topo(grid_y)
$ns trace-all $nf

#Open the Trace file
set tf [open dump_files/t2_main.tr w]
$ns trace-all $tf
...
for {set i 0} {$i < $topo(nn)} {incr i} {
    ...
    $ns idrm_set $topo($i,my_domain) $topo($i,my_identity) $topo(ng) $topo(dnn) $file_name $topo(eb) $topo(ib) $topo(dynamic) $topo(percentage_gateway) $topo(schannel) 12
